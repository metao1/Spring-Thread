...
public class PriorityDequeue<E> extends AbstractQueue<E> implements Deque<E>, Serializable {
    ...
    public PriorityDequeue(int initialCapacity)
    {
        this(initialCapacity, null);
    }

    public PriorityDequeue(int initialCapacity, Comparator<? super E> comparator)
    {
    ...
    public PriorityDequeue(Collection<? extends E> c)
    {
        ...
        else if (c instanceof PriorityDequeue<?>)
        {
            PriorityDequeue<? extends E> pq = (PriorityDequeue<? extends E>) c;
    ...
    @SuppressWarnings("unchecked")
    public PriorityDequeue(PriorityDequeue<? extends E> c)
    {
    ...
    @SuppressWarnings("unchecked")
    public PriorityDequeue(SortedSet<? extends E> c)
    {
        this.comparator = (Comparator<? super E>) c.comparator();
        addAll(c);
    }

    private void initFromPriorityDeque(PriorityDequeue<? extends E> c)
    {
        if (c.getClass() == PriorityDequeue.class)
        {
            deque = c.toArray();
            size = c.size();
        }
    ...
    private final class Itr implements Iterator<E>
    {
        ...
        public void remove()
        {
            ...
            if (lastRet != -1)
            {
                boolean decrementCounter = PriorityDequeue.this.removeAtIter(lastRet, desc);
                lastRet = -1;
                if (decrementCounter)
                {
                    cursor--;
        ...
        {
            if (cursor < size)
                return (E) PriorityDequeue.this.deque[lastRet = cursor++];
            throw new NoSuchElementException();
        }
